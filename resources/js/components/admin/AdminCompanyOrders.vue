  ...
  <div>
    ...
    <main style="margin-top: 58px">
      ...
      <div class="container pt-4">
        <div
          v-for="order in companiesOrders"
          :key="order.id"
          class="col-md-auto"
        >
          <CompanyOrderItem :id="order.id" :order="order" />
        </div>
      </div>
      ...
      <nav aria-label="Page navigation example" v-if="getPagesArray.length > 2">
        <ul class="pagination justify-content-center mt-5">
          <li>
            <nuxt-link
              :to="'/admin/companyOrders?page=' + 1 + '&id=' + company_id"
          ...
          <li class="page-item" v-for="page in pages_array" :key="page.id">
            ...
            <nuxt-link
              v-else
              :to="'/admin/companyOrders?page=' + page + '&id=' + company_id"
              class="page-link"
              >{{ page }}
            </nuxt-link>
          </li>
          ...
          <li>
            <nuxt-link
              :to="
                '/admin/companyOrders?page=' + pages_count + '&id=' + company_id
              "
              class="page-link ms-3"
              >Last
            </nuxt-link>
...
<script>
import CompanyOrderItem from "./CompanyOrderItem.vue";
...
export default {
  ...
  setup() {
    ...
    watch(route, () => {
      getCompaniesOrders(route.params.company_id, route.params.page);
    });
    onMounted(() => {
      getCompaniesOrders(route.params.company_id, 1);
    });
